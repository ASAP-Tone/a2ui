...
class SalesforceAgentExecutor(AgentExecutor):
    """Salesforce AgentExecutor Example."""

    def __init__(self, base_url: str):
        self.ui_agent = SalesforceAgent(base_url=base_url, use_ui=True)
        self.text_agent = SalesforceAgent(base_url=base_url, use_ui=False)
    ...
    async def execute(
        ...
        if ui_event_part:
            ...
            if action == "view_record":
                record_id = ctx.get("id", "Unknown")
                query = f"ACTION: View record with ID {record_id}"
            elif action == "delete_record":
                 record_id = ctx.get("id", "Unknown")
                 query = f"ACTION: Delete record with ID {record_id}"
            elif action == "edit_record":
                 record_id = ctx.get("id", "Unknown")
                 query = f"ACTION: Edit record with ID {record_id}"
            else:
