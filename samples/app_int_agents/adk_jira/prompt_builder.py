...
AGENT_INSTRUCTION = """
    ...
    1.  **For finding issues (e.g., "Show me my open tickets"):**
        ...
        b.  After receiving the data:
            i.   If the tool returns a **single issue**, you MUST use the `ISSUE_DETAIL_EXAMPLE` template.
    ...
    2.  **For handling actions (e.g., "Assign this to me"):**
...
def get_ui_prompt(base_url: str, examples: str) -> str:
    ...
    -   **For finding issues (e.g., "Find tickets in project PROJ"):**
        a.  You MUST call the `get_jira_issues` tool.
        b.  If the tool returns a **single issue**, you MUST use the `ISSUE_DETAIL_EXAMPLE` template. Populate the `dataModelUpdate.contents` with the issue's details (key, summary, status, priority, etc.).
